...
class NNTanh(nn.Module):
    ...
    def forward(self, input_data : torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.network(input_data.float())
...
class Siren_d(nn.Module):
    ...
    def build_network(
        ...
        # Input layer
        network.append(
            SineLayer_d(n_in, n_hidden[0], is_first=True, omega_0=first_omega_0)
        )

        # Hidden layers
        for layer_i, layer_j in zip(n_hidden, n_hidden[1:]):
            network.append(
                SineLayer_d(layer_i, layer_j, is_first=False, omega_0=hidden_omega_0)
            )
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_features, activation=nn.ReLU):
...
class Gaussian(ActivationFunction):
    ...
    def __init__(self, std=0.5):
        super().__init__()
        self.config["variance"] = std**2
        self.config["frequency"] = 1/std
        
    def forward(self, x):
        return torch.exp(-x**2/(2*self.config["variance"]))




class NNGaussian(nn.Module):
    def __init__(self, n_in: int, n_hidden: List[int], n_out: int, var ) -> None:
        """
        Discription:
        Constructing a NN with sigmoid activation function
        Args:
    ...
    def build_network(
        ...
        for layer_i, layer_j in zip(architecture, architecture[1:]):
            network.append(nn.Linear(layer_i, layer_j))
            network.append(Gaussian(std = self.var))
        network.pop()  # get rid of last activation function
        return nn.Sequential(*network)
...
class SincActivation(nn.Module):
    ...
    def forward(self, x):
        # 避免除以零的情况
        return torch.where(torch.abs(x)< thre, torch.ones_like(x), torch.sin(self.fre * x) / (self.fre * x))
    



class NNSinc(nn.Module):
    def __init__(self, n_in: int, n_hidden: List[int], n_out: int, fre: float ) -> None:
        """
        Discription:
        Constructing a NN with sigmoid activation function
        Args:
    ...
    def build_network(
        ...
        for layer_i, layer_j in zip(architecture, architecture[1:]):
            network.append(nn.Linear(layer_i, layer_j))
            network.append(SincActivation(fre = self.fre))
        network.pop()  # get rid of last activation function
        return nn.Sequential(*network)
